...
class MinimalSubscriber(Node):
    ...
    def listener_callback(self, msg):
        ...
        #filtre pour détecter le robot
        hsv1 = cv2.cvtColor(current_frame, cv2.COLOR_RGB2HSV)
        lower = np.array([100, 60, 150], dtype=np.uint8)
        upper = np.array([200, 90, 255], dtype=np.uint8)
        seg0 = cv2.inRange(hsv1, lower, upper)
        pixel_blanc_x = []
        pixel_blanc_y = []
        ...
        for i in range(len(seg1)):
            for j in range(len(seg1[0])):
                if seg0[i,j] == 255:
                    pixel_blanc_x.append(i)
                    pixel_blanc_y.append(j)
        pos_x_pelle = int(np.sum(pixel_blanc_x)/len(pixel_blanc_x))
        pos_y_pelle = int(np.sum(pixel_blanc_y)/len(pixel_blanc_y))
        position_pelle =(pos_x_pelle, pos_y_pelle)
        
        angle = self.angle(position_pelle, position_robot)
